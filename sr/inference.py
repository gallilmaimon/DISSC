...
def init_worker(queue, arguments):
    ...
    if a.sample_df:  # used to only perform specific conversions for space and time saving
        df = pd.read_csv(a.sample_df, index_col=0)
        if a.target_speakers:
            df = df[df.syn_trgt.isin(a.target_speakers)]
    ...
    if a.code_file is not None:
        ...
        base_path = h.test_base_path if a.data_path is None else a.data_path
        file_list = parse_manifest(a.input_code_file, base_path)
        if a.unseen_speaker:
            with open(a.id_to_spkr, 'rb') as f:
        ...
        dataset = CodeDataset(file_list, -1, h.code_hop_size, h.n_fft, h.num_mels, h.hop_size, h.win_size,
                              ...
                              pad=a.pad, id_to_spkr=id_to_spkr, eval_mode=a.eval_mode, unseen_speakers=a.unseen_speaker)
...
def inference(item_index):
    ...
    if a.sample_df is None and not a.unseen_speaker:  # for unseen speakers we can not do resynthesis, only VC
    ...
    if h.get('multispkr', None) and a.vc:
        if a.target_speakers is None:
            ...
            local_spkrs = spkrs
        if a.sample_df:
            cur_name = fname_out_name.split('_mic2')[0]
            local_spkrs = [dataset.spkr_to_id[i] for i in df[df.syn_sample == cur_name].syn_trgt.unique()]
    ...
    if gt_audio is not None and a.sample_df is None:
...
def main():
    ...
    parser.add_argument('--unseen_speaker', action='store_true', help='This means we use as input for conversion an unseen speaker')
    parser.add_argument('--id_to_spkr', default=None, type=Path, help='Path for id_to_spkr pickle dict. Used for unseen speakers')
    parser.add_argument('--sample_df', default=None, type=Path)
